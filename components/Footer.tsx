...
import { Facebook, Instagram, Phone, Mail, MapPin, Heart, Map } from 'lucide-react';
...
const Footer: React.FC<FooterProps> = ({ onNavigate }) => {
  const nav = (href: string) => navigate(href, onNavigate);
  ...
  return (
    <footer className="relative bg-deep-navy text-white pt-24 pb-10 overflow-hidden">
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-t from-deep-navy via-deep-navy/90 to-transparent" />

      <div className="container mx-auto px-4 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20">

          <div className="flex flex-col items-center md:items-start">
            <button onClick={() => nav('/')} aria-label="Página inicial">
              <img
                src="/images/logo.png"
            ...
            <div className="flex gap-4">
              <a href="https://facebook.com/coracaodailha" target="_blank" rel="noreferrer" className="w-12 h-12 rounded-full border-2 border-white/10 hover:border-lime flex items-center justify-center transition-all hover:bg-lime hover:text-deep-navy group" aria-label="Facebook">
                <Facebook size={22} className="group-hover:scale-110 transition-transform" />
              </a>
              <a href="https://instagram.com/coracaodailha" target="_blank" rel="noreferrer" className="w-12 h-12 rounded-full border-2 border-white/10 hover:border-lime flex items-center justify-center transition-all hover:bg-lime hover:text-deep-navy group" aria-label="Instagram">
          ...
          <div className="text-center md:text-left">
            ...
            <ul className="space-y-4 text-gray-300 font-medium">
              <li><button onClick={() => nav('/')} className="hover:text-lime transition-colors">Página Inicial</button></li>
              <li><button onClick={() => nav('/#sobre')} className="hover:text-lime transition-colors">A Pousada</button></li>
              <li><button onClick={() => nav('/quem-somos')} className="hover:text-lime transition-colors">Quem Somos</button></li>
              <li><button onClick={() => nav('/#experiencia')} className="hover:text-lime transition-colors">Ilha do Mel</button></li>
              ...
              <li>
                <button
                  onClick={() => nav('/guia-ilha-do-mel')}
                  className="flex items-center gap-2 font-black text-ocean hover:text-lime transition-colors"
                >
                  <Map size={14} />
                  Guia da Ilha do Mel
                </button>
          ...
          <div className="text-center md:text-left">
            ...
            <div className="space-y-4 bg-white/5 p-6 rounded-3xl border border-white/5 mb-8">
            ...
            <h4 className="text-lime text-xs font-black uppercase tracking-[0.3em] mb-4">Legal</h4>
            <ul className="space-y-3 text-gray-400 text-sm">
              <li><button onClick={() => nav('/politica-privacidade')} className="hover:text-lime transition-colors">Política de Privacidade</button></li>
              <li><button onClick={() => nav('/politica-devolucao')} className="hover:text-lime transition-colors">Política de Cancelamento</button></li>
              <li>
                <button onClick={() => nav('/mapa-do-site')} className="hover:text-lime transition-colors flex items-center gap-1">
                  <Map size={14} />
                  Mapa do Site
                </button>
              </li>
        ...
        <div className="pt-10 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-[10px] text-gray-500 uppercase tracking-[0.2em] font-bold text-center md:text-left">
            © 2026 Pousada Coração da Ilha. Todos os direitos reservados.
          </div>
